<?php 
    ...
    if(isset($_POST['add-dog-form-btn']) && isset($_FILES['dog_photo']) && $_FILES['dog_photo']['error'] == 0){
        ...
        if(!array_filter($dogErrors)){
            ...
            //prepared statement
            $stmt = $conn->prepare("INSERT INTO cats(pet_id, is_leash_trained, dog_size, photo_name, photo_data, photo_size, photo_type) VALUES(?, ?, ?, ?, ?, ?, ?, ?)");

            if($stmt){
                //bind parameters
                $stmt->bind_param("iisssis", $pet_last_inserted_id, $leashTrained, $dogSizeInput, $fileName, $fileData, $fileSize, $fileType);
