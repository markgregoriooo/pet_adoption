<?php
  ...
  //include mysql query
  include('mysql_queries/cat_adopter_query.php');
  ...
  if (isset($_GET['id'])) {
     ...
     $stmt = $conn->prepare(
        "SELECT * FROM cats 
        INNER JOIN pets ON cats.pet_id = pets.pet_id 
        WHERE pets.is_deleted = FALSE AND cats.cat_id = ? 
     ...
     //execute
     if(!$stmt->execute()){
        // if there is/are error, secho the error
     ...
     $_SESSION['pet_id'] = $cat['pet_id']; //  store the pet id to the session var  
     // Close the statement 
     $stmt->close();
  }
?>
                ...
                <div class="row mt-3 d-flex justify-content-center">
                  ...
                  <!-- status -->
                   ...
                   <div class="text-center mt-5 mb-4">
                      ...
                      <button class="btn btn-success w-25" id="adoption-form-btn" name="cat-adoption-form-btn">Submit</button>
                   </div>
              </div>
              <!-- end div row -->
            </form>
