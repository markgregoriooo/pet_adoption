...
if (isset($_POST['editAdptdPetForm-btn'])) {
    ...
    if (empty($_POST['adopter-fullname'])) {
        ...
        // regex
        if (!preg_match("/^[a-z]{1,20}\s([a-zA-Z]{1,20}\s*)+$/i", $fNameInput)) { //regex for full name
    ...
    if (empty($_POST['adopter-email-address'])) {
        ...
        // regex
        if (!filter_var($emailAddInput, FILTER_VALIDATE_EMAIL)) { //regex for email address
    ...
    if (empty($_POST['adopter-address'])) {
        ...
        // regex
        if (!preg_match("/^([a-z]{1,10}\s*)+[,]\s*([a-z]{1,10}\s*[\w]*)+[,]\s*([a-z]{1,10}\s*)+[,]\s*([a-z]{1,10}\s*)+$/i", $addressInput)) { //regex for address
    ...
    if (empty($_POST['adopter-contact'])) {
        ...
        // regex
        if (!preg_match("/^[0]\d{3}[-]\d{3}[-]\d{4}$/i", $contactNumInput)) { //regex for contact no
